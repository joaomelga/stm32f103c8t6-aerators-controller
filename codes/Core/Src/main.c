...
int main(void) {
	...
	BOOTING_MSG, animationTurn = 0;
	unsigned int lastMsgTimestamp = - MIN_MSG_GAP, timePassedSinceLastMsg = 0,
			lastAnimationTimestamp = HAL_GetTick(),
			timePassedSinceLastAnimation = 0, lastMotorToggleTimestamp =
					HAL_GetTick(), timeSinceLastMotorToggle = 0;
	...
	// Print ligth
	SSD1306_DrawLine(LIGHT_ICON_X + 0, LIGHT_ICON_Y + 0, LIGHT_ICON_X + 24,
	LIGHT_ICON_Y + 24, 1);
	SSD1306_DrawLine(LIGHT_ICON_X + 24, LIGHT_ICON_Y + 0, LIGHT_ICON_X + 0,
	LIGHT_ICON_Y + 24, 1);
	SSD1306_DrawLine(LIGHT_ICON_X + 12, LIGHT_ICON_Y + 0, LIGHT_ICON_X + 12,
	LIGHT_ICON_Y + 24, 1);
	SSD1306_DrawLine(LIGHT_ICON_X + 0, LIGHT_ICON_Y + 12, LIGHT_ICON_X + 24,
	LIGHT_ICON_Y + 12, 1);
	...
	while (1) {
		...
		if (timePassedSinceLastAnimation > ANIMATION_GAP) {
			...
			case 0:
				...
				sprintf(displayText, "%i.%iK", ldrResistence / 1000, (ldrResistence % 1000 / 100));
				SSD1306_GotoXY(LIGHT_ICON_X, TEXT_Y + 12);
				SSD1306_Puts(displayText, &Font_7x10, 1);
